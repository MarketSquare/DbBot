...
DbBot is a Python script to serialize Robot Framework produced test run
results, i.e. output.xml files, into a SQLite database. This way the future
Robot Framework related tools and plugins will have a unified storage for the
test run results.
...
Robot Framework version 2.7.4 or later is recommended as versions prior to
...
'.py' file extension ('bin/dbbot' -> 'bin/dbbot.py').
...
suites and test cases are stored by default. However, keywords are not stored
by default as it might take tens of seconds for massive test runs. Keywords
can be stored by using -k or --also-keywords flag.
...
You may also want to append the DbBot root directory to your PYTHONPATH if
you are developing something that uses the classes.

License
-------

DbBot is released under the `Apache License, Version 2.0
<http://www.tldrlegal.com/license/apache-license-2.0>`__.
